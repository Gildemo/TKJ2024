...
int intcounter = 0;
int *intpointer = &intcounter;
int mflag1 = 0;
int mflag2 = 0;
//MPU tulosten muuttujien alustus
float ax, ay, az, gx, gy, gz;
//vastaanotettavan viestin muuttujien alustus
char uartBuffer[30] = {};
char piste = '.';
char viiva = '-';
char vali = ' ';
...
void buttonFxn(PIN_Handle handle, PIN_Id pinId) {
       ...
       *bcpointer +=1;
       ...
       if (buttoncounter == 1){
           ...
           strcpy(viesti,".\r\n\0");
           System_printf("buttoncounter: \0%d\n", buttoncounter);
           System_printf("uartBuffer kohdassa 0: %c\n", uartBuffer[0]);
           System_flush();
       }
       if (buttoncounter == 2){
...
/* Task Functions */
static void uartFxn(UART_Handle uart, void *buf, size_t len) {


       if(strncmp(buf,"-",1) == 0){
           uartBuffer[intcounter] = viiva;
           intcounter++;
       }
       if(strncmp(buf,".",1) == 0){
           uartBuffer[intcounter] = piste;
           intcounter++;
       }
...
Void buzzerTaskFxn(UArg arg0, UArg arg1) {
  ...
  while (1) {
      if (programState == READING){
          System_printf("bufferin arvo kohdassa 0: %c\n", uartBuffer[0]);
          System_flush();
          if (stopcounter == 4){
              programState = IDLE;
              PIN_setOutputValue( ledHandle, Board_LED0, 0 );
          ...
          else if (uartBuffer[loopcounter] == piste){
              buzzerOpen(hBuzzer);
          ...
          else if (uartBuffer[loopcounter] == viiva){
              buzzerOpen(hBuzzer);
              buzzerSetFrequency(2000);
              Task_sleep(100000 / Clock_tickPeriod);
              buzzerClose();
          ...
          else if (uartBuffer[loopcounter] == vali){
